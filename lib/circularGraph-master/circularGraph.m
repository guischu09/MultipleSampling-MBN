...
%         'UserData',this);
    ...
    %%
      ...
      for i = 1:length(adjacencyMatrix)
        this.Node(i) = node(cos(t(i))*dF,sin(t(i))*dF);
      ...
      %% Guilherme Edit - 
      % In case of negative connectivity values      
      v = abs(v);
      lineWidthCoef = 2;
      pyColors = importdata(this.ColorMap); pyColors = pyColors.data;
      Corrvals = pyColors(:,2);
      colors = pyColors(:,3:end);
...
%       lineWidthCoef = 5;
      ...
      for i = 1:length(v)
        ...
        if row(i) ~= col(i)
          ...
          rvalue = adjacencyMatrix(row(i),col(i));  
          logic_vec = abs(rvalue-Corrvals) < 0.00001;          
          cc = colors(logic_vec,:);
          
          % if repeated colors avoid that simply by doing:
          cc = cc(1,:);
          ...
          if abs(row(i) - col(i)) - length(adjacencyMatrix)/2 == 0 
            ...
            u = [cos(t(row(i)))*dF;sin(t(row(i)))*dF];
            v = [cos(t(col(i)))*dF;sin(t(col(i)))*dF];
      ...
      extent = max(extent(:));
      fudgeFactor = 0.5; % Not sure why this is necessary. Eyeballed it.
