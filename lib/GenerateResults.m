function GenerateResults(output,setup,Labels,atlas_filename,CoordsFilename,clobber)
...
% [GTM_data,DataHeader] = ComputeGraphMeasures(output,setup);
[GTM_data,DataHeader,GTM_data2,DataHeader2] = ComputeGraphMeasures_v2(output,setup);
...
% Plot boxplot with seaborn lib in python
% pyFilename= fullfile(pwd,'lib','plot_LocalGraphMeasures.py');
%
% for forMat = 1:length(setup.OUTPUT_FOMAT_VEC)
%     OutputFormat = setup.OUTPUT_FOMAT_VEC{forMat};
%
    ...
    case 'Conventional'
        ...
        for c = 1:size(output.DATA,1)
            ...
            for forMat = 1:length(setup.OUTPUT_FOMAT_VEC)
                ...
                OutputFormat = setup.OUTPUT_FOMAT_VEC{forMat};
                circularGraph(network,'Colormap',cMap,'Label',strrep(Labels,'_','-'));
    ...
    if isfield(setup,'plotMatrix')
        ...
        switch setup.plotMatrix
            ...
            case 'MS_Scheme'
                ...
                for forMat = 1:length(setup.OUTPUT_FOMAT_VEC)
                    pythonFilename1 = fullfile(pwd,'lib','plotScript.py');
                    OutputFormat = setup.OUTPUT_FOMAT_VEC{forMat};
                    plot_type = 'Undirected_Network';
                    colormap = 'jet';
                    
                    [command1] = Heatmap_pycommand(pythonFilename1,inMat,atlas_filename,CoordsFilename,output.DATA{u,2}, ...
                        OutputFormat, SubDir2, plot_type,outputFilename,colormap);
                    o = system(command1);
