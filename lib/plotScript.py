...
@click.command()
@click.option('--vmin', default = 0.0, help = 'Vmin for color map.')
@click.option('--vmax', default = 1.0, help = 'Vmax for color map.')
@click.option('--size_cte', default = 1, help = 'Node size constant')
@click.option('--atlas', default="", help='Atlas filename.')
@click.option('--coor', default="", help='Coordinates CSV filename.')
...
@click.option('--plot_type', default = "Undirected_Network", help = 'Directed_Network / Undirected_Network')
@click.option('--cmap', default = "jet", help = 'jet / binary')


################## Input Options ############################

def main(vmin, vmax, size_cte, atlas, coor, corr, out_format, out_bp, out_hm, out_fd, plot_type,cmap):
   


    output_brainplot = os.path.join(out_fd,out_bp + "." + out_format)
    ...
    def plot3D():
        ...
        if plot_type == 'Directed_Network':        
            strength_diff = np.zeros((len(vois),len(vois)))
            abs_strength_diff = np.zeros((len(vois),len(vois)))          
            for u, voi1 in enumerate(vois):
                for v, voi2 in enumerate(vois):            
                    strength_diff[u,v] = strength[voi1] - strength[voi2]
                    abs_strength_diff[u,v] = abs(strength[voi1] - strength[voi2])
    
        ############ Directed Network  Feature END ##################################    
    
        ##### This block of code creates a matplotlib figure and insert the atlas in it. Also modifies the fig apparence #####
        fig = plt.figure(figsize = (25, 24))
        ax = fig.gca(projection = "3d")
        
        # Fancy indexing: 'verts[faces]' to generate a collection of triangles
        mesh = Poly3DCollection(verts[faces], linewidths = 0.01, alpha = .05, zorder = -1)
        mesh.set_facecolor('gray')
        ax.add_collection3d(mesh)
        adjust_vis = 1.5;
        ax.set_xlim([0, max(x) * adjust_vis])
        ax.set_ylim([0, max(y)])
        ax.set_zlim([0, max(z) * adjust_vis])
        ...
        if plot_type == 'Undirected_Network':       
        
            # Creates the lines on figure for each pair
            for p in pairs:
                ...
                if correlation != 0:
                    ...
                    color = mpl.colors.rgb2hex(m.to_rgba(correlation))
        
                    ax.plot([xinit, xfinal], [yinit, yfinal], [zinit, zfinal], color = color)
                                
        ############ CONNECTIONS WITH NO DIRECTION END   ####################################################
        ...
        elif plot_type == 'Directed_Network':
                                                   
            # Creates the arrows on figure for each pair
            for p in pairs:
                correlation = corr_matrix[p[0]][p[1]]
                ...
                if correlation != 0:
                    ...
                    if strength[p[1]] > strength[p[0]]:
                        direction = coordinates[p[0]] - coordinates[p[1]]
                                       
                        xinit = coordinates[p[1]][0]
                        yinit = coordinates[p[1]][1]
                        zinit = coordinates[p[1]][2]
            
                        xfinal = direction[0]
                        yfinal = direction[1]
                        zfinal = direction[2]
                                        
                        ax.quiver(xinit,yinit,zinit,xfinal,yfinal,zfinal,arrow_length_ratio=.09,color = color)    
                    
                    else:
                        direction = coordinates[p[1]] - coordinates[p[0]]                    
        ...
        ############ CONNECTIONS WITH DIRECTION END ####################################################                        
                
        # Creates the nodes on figure
        for i in range(len(vois)):
            ax.scatter(Xn[i], Yn[i], Zn[i], color="white", s = sizes[i]**2.3 + size_cte, edgecolor = 'black', linewidth = '1', zorder = 0)
        
        # Saves figure
        fig.savefig(output_brainplot, bbox_inches = 'tight', pad_inches = 0)
    ...
    def plotHeatmap():     
        ...
        #sns.heatmap(corr_matrix, vmin = vmin, vmax = vmax, cmap = cmap,linecolor = 'k',linewidths = 0.05, xticklabels = False)
        sns.heatmap(corr_matrix, mask = mask, vmin = vmin, vmax = vmax, cmap = cmap,linecolor = 'k',linewidths = 0.05, xticklabels = True)
